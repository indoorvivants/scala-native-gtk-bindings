...
class MemoryOutputStream(raw: Ptr[GMemoryOutputStream]) extends OutputStream(raw.asInstanceOf), PollableOutputStream, Seekable:
  ...
  def getDataSize(): ULong = g_memory_output_stream_get_data_size(this.raw.asInstanceOf).value

  def getSize(): ULong = g_memory_output_stream_get_size(this.raw.asInstanceOf).value
...
object MemoryOutputStream:
  def apply(data : Ptr[Byte], size : ULong, realloc_function : GReallocFunc, destroy_function : GDestroyNotify): MemoryOutputStream = new MemoryOutputStream(g_memory_output_stream_new(gpointer(data), gsize(size), realloc_function, destroy_function).asInstanceOf)
